...
async def create_user(user: _schemas.UserCreate, db: _orm.Session = _fastapi.Depends(_services.get_db)):
    ...
    user = await _services.create_user(user, db)  # создание пользователя, добавление в бд т возвращение экземпляра модели

    return await _services.create_token(user)  # выдача токена и авторизация юзера
...
async def delete_lead(lead_id: int,
                      user: _schemas.User = _fastapi.Depends(_services.get_current_user),
                      db: _orm.Session = _fastapi.Depends(_services.get_db)):
    return await _services.delete_lead(lead_id, user, db)
